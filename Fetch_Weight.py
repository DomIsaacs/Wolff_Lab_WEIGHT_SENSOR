...
address=('10.90.125.50', 1900) #define server IP and port
...
while(1):
    ...
    client_socket.sendto(str.encode(data), address) #Send the data request
 
    try:
 
        rec_data, addr = client_socket.recvfrom(2048) #Read response from arduino
        temp = float(rec_data) #Convert string rec_data to float temp
        print("Rat Weight: ", temp, "grams.")# Print the result
